...
let prsnWithJavascript = persons.map(prsnObj =>
    {
        if (prsnObj.tags === "javascript")
            return { "Person with javascript tag" : prsnObj.name}
        else
            return "" }).filter(p => p!="")
console.log (prsnWithJavascript)

//2. List the same on person using java and put programmer after their name, change the name key to Developer

let prsnWithJava = persons.map(prsnObj =>
    {
        if (prsnObj.tags === "java")
            return { "Person with java tag" : prsnObj.name + " Programmer "}
        else
            return "" }).filter(p => p!="")
console.log (prsnWithJava)

//3. If we have anyone with tag python
let prsnWithPython = persons.map(prsnObj =>
    {
        if (prsnObj.tags === "python")
            return { "Person with python tag" : prsnObj.name}
        else
            return "" }).filter(p => p!="")
console.log (prsnWithPython)

//4. Find the number of unique tags and their count present

let prsnCount = persons.reduce((prevVal, currValue) =>
{
    prevVal[currValue.tags] = prevVal[currValue.tags] ? prevVal[currValue.tags] +1 : 1;
    return prevVal;
}, new Set())

console.log(prsnCount);
